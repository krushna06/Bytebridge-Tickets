		...
		try {
			...
			for (const stats of statsMessages) {
				try {
					...
					const settings = await client.prisma.guild.findUnique({ where: { id: stats.guildId } });
					...
					const interval = setInterval(async () => {
						try {
							...
							const updatedEmbed = await statsCommand.generateStatsEmbed({
								guild,
								options: { getString: () => '7d' },
							}, settings);
							await message.edit({ embeds: [updatedEmbed] });
						} catch (error) {
							console.log('Error updating stats message:', error);
					...
					await client.prisma.statsMessage.delete({ where: { guildId: stats.guildId } });
