...
module.exports = class LeaderboardSlashCommand extends SlashCommand {
	...
	async run(interaction) {
		...
		await interaction.reply({
			content: 'Fetching leaderboard or stats...',
			ephemeral: false,
		});
		...
		const {
			avgResolutionTimePerUser, avgResponseTimePerUser,
		} = stats || {};
		...
		const createLeaderboardEmbed = async (type, user) => {
			...
			if (type === 'overall') {
				if (user) {
					...
					const {
						avgGuildResponseTime, avgGuildResolutionTime, avgGuildFeedback,
