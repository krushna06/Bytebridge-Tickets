...
module.exports = class StatsPanelSlashCommand extends SlashCommand {
	...
	async run(interaction) {
		...
		await interaction.reply({
			content: 'Fetching stats and profiles...',
			ephemeral: false,
		});
		...
		const createEmbed = async (avgResolutionTimePerUser, avgResponseTimePerUser, totalTickets, profiles) => {
			...
			profileEmbed.addFields(
				{
					name: 'Guild Avg Response Time',
					value: `${guildAvgResponseTime} seconds`,
					inline: true,
				},
				{
					name: 'Guild Avg Resolution Time',
					value: `${guildAvgResolutionTime} seconds`,
					inline: true,
				},
			);
		...
		try {
			...
			if (statsMessage) {
				await statsMessage.edit({ embeds: [await createEmbed(initialStats.avgResolutionTimePerUser, initialStats.avgResponseTimePerUser, initialStats.totalTickets, profiles)] });
			}
		} catch (error) {
		...
		if (!statsMessage) {
			const guildChannel = await client.channels.fetch('1292032641125843005');
			statsMessage = await guildChannel.send({ embeds: [await createEmbed(initialStats.avgResolutionTimePerUser, initialStats.avgResponseTimePerUser, initialStats.totalTickets, profiles)] });
		}
		...
		const updateInterval = setInterval(async () => {
			...
			if (updatedStats) {
				await statsMessage.edit({ embeds: [await createEmbed(updatedStats.avgResolutionTimePerUser, updatedStats.avgResponseTimePerUser, updatedStats.totalTickets, updatedProfiles)] });
			}
		}, 60000);  // 1 minute
